...
//ERROR HANDLER
errorHandler = function(res, statusText, e) {
	if (_.isFunction(o.error)) { o.error(res, statusText, e); }
	dfd.reject(e);
}
...
//REQUEST
	var req = (ssl ? https : http).request(options, function(res) {
		...
		res.on('end', function() {
			...
			statusText = statusCode + ": " + http.STATUS_CODES[statusCode];

			if (o.dataType === 'json') {
				//replace control characters
				try { data = JSON.parse(data.replace(/[\cA-\cZ]/gi,'')); }
				catch(e){ return errorHandler(res, statusText, data); }
			}
			if (statusCode < 400) {
				if ( _.isFunction(o.success)) { o.success(data, statusText, res); }
				dfd.resolve(data);
			} else {
				errorHandler(res, statusText, data);
			}
		});
	});
...
//ERROR
	req.on('error', function(e) {
		errorHandler(null, "Connection Error", e);
	});
